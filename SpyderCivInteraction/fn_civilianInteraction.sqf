...
switch (_operation) do {
	...
	case "loadData": {
		...
		lbAdd [SCI_QuestionList, "Onde você mora?"];lbSetData [SCI_QuestionList, 0, "Home"];
		lbAdd [SCI_QuestionList, "Que cidade você mora em"];lbSetData [SCI_QuestionList, 1, "Town"];
		lbAdd [SCI_QuestionList, "Você viu alguma IED de ultimamente?"];lbSetData [SCI_QuestionList, 2, "IEDs"];
		lbAdd [SCI_QuestionList, "Você já viu qualquer atividade insurgente ultimamente?"];lbSetData [SCI_QuestionList, 3, "Insurgents"];
		lbAdd [SCI_QuestionList, "Você sabe a localização de quaisquer esconderijos insurgentes?"];lbSetData [SCI_QuestionList, 4, "Hideouts"];
		lbAdd [SCI_QuestionList, "Você viu algum comportamento estranho ultimamente?"];lbSetData [SCI_QuestionList, 5, "StrangeBehavior"];
		lbAdd [SCI_QuestionList, "Você apoia as forças de segurança?"];lbSetData [SCI_QuestionList, 6, "Opinion"];
		lbAdd [SCI_QuestionList, "Qual é a sua opinião sobre nossas forças nesta área?"];lbSetData [SCI_QuestionList, 7, "TownOpinion"];
		...
		if !(_hostile) then {
			if (floor random 100 < (3 * _asked)) then {
				...
				if (floor random 70 < (_asked * 5)) then {
					_response1 = format [" *%1 parece visivelmente irritado*", name _civ];
					_response2 = format [" *%1 parece desinteressado na conversa *", name _civ];
					_response3 = " Por favor, deixe-me em paz agora.";
					_response4 = " Eu não quero mais falar com você.";
					_response5 = " Posso ir agora?";
			...
			if (floor random 100 < (8 * _asked)) then {
				["UpdateHostility", [_civ, 10]] call MAINCLASS;
				if (floor random 70 < (_asked * 5)) then {
					_response1 = format [" *%1 parece ansioso*", name _civ];
					_response2 = format [" *%1 parece distraído*", name _civ];
					_response3 = " Você ainda não esta satisfeito?";
					_response4 = " Você faz muitas perguntas.";
					_response5 = " Você precisa sair agora.";
					_response = [_response1, _response2, _response3,_response4, _response5] call BIS_fnc_selectRandom;
					SCI_ResponseList ctrlSetText ((ctrlText SCI_ResponseList) + _response);
				};
			};
	...
	case "getActivePlan": {
		...
		switch (toLower _activeCommand) do {
			case "alive_fnc_cc_suicide": {
				_activePlan1 = "a realização de um atentado suicida";
				_activePlan2 = "cintos com explosivos";
				_activePlan3 = "planejando um atentado";
				_activePlan4 = "se preparando para bombardear suas forças";
				_activePlan5 = "prestes a bombardear suas forças";
			...
			case "alive_fnc_cc_suicidetarget": {
				_activePlan1 = "planejando realizar um atentado suicida";
				_activePlan2 = "cintos com explosivos";
				_activePlan3 = "planejando um atentado";
				_activePlan4 = "se preparando para bombardear suas forças";
				_activePlan5 = "prestes a bombardear suas forças";
			...
			case "alive_fnc_cc_rogue": {
				_activePlan1 = "guardar alguma arma em sua casa";
				_activePlan2 = "estocando armas";
				_activePlan3 = "pensando em filmar uma patrulha";
				_activePlan4 = "procurando patrulhas para atirar";
				_activePlan5 = "pago para atirar em suas forças";
				_result = [_activePlan1,_activePlan2,_activePlan3,_activePlan4,_activePlan5] call BIS_fnc_selectRandom;
			};
			...
			case "alive_fnc_cc_sabotage": {
				_activePlan1 = "planejando sabotar um edifício";
				_activePlan2 = "explodir um prédio";
				_activePlan3 = "explosivos plantados nas proximidades";
				_activePlan4 = "se preparando para plantar explosivos";
				_activePlan5 = "pago para atirar em suas forças";
				_result = [_activePlan1,_activePlan2,_activePlan3,_activePlan4,_activePlan5] call BIS_fnc_selectRandom;
			};
			case "alive_fnc_cc_getweapons": {
				_activePlan1 = "recuperação de armas de um depósito de armas nas proximidades";
				_activePlan2 = "pensando em juntar os insurgentes";
				_activePlan3 = "se preparando para ir a um centro de recrutamento de insurgentes";
				_activePlan4 = "se preparando para recuperar armas de um cache";
				_activePlan5 = "pago para atacar suas forças";
				_activePlan6 = "forçado a juntar-se aos insurgentes";
				_activePlan7 = "se preparando para atacar suas forças";
				_result = [_activePlan1,_activePlan2,_activePlan3,_activePlan4,_activePlan5] call BIS_fnc_selectRandom;
			};
		};
	};
