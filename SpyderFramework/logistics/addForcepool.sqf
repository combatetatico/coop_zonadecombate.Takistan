...
Examples:
(begin example)
["BLU_F", 20] call SF_fnc_addForcepool; //-- Add 20 forcepool to NATO
["BLU_F", -20] call SF_fnc_addForcepool; //-- Subtract 20 forcepool to NATO
[[] call SF_fnc_getFactionMostPlayers, 20] call SF_fnc_addForcepool; //-- Add 20 forcepool to the faction with the most players
(end)
...
if ((isNil {[ALIVE_globalForcePool, _faction] call ALIVE_fnc_hashGet}) or (isNil "_faction")) exitWith {
	["SF_fnc_addForcepool: Faction %1 does not have an existing forcepool", _faction] call ALIVE_fnc_dump;
	false;
};
...
["SF_fnc_addForcepool: %1 forcepool added to faction %2", _amount, _faction] call ALIVE_fnc_dump;