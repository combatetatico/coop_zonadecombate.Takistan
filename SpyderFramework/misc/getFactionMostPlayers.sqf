/* ----------------------------------------------------------------------------
Function: SF_fnc_getFactionMostPlayers

Description:
Gets the faction that contains the most players
...
Examples:
(begin example)
_faction = [] call SF_fnc_getFactionMostPlayers;
(end)
...
for "_i" from 0 to (count _factionData) do {
	...
	if (_factionCount == _playerFaction) exitWith {
		["SF_fnc_getFactionMostPlayers: Returning faction %1", _faction] call ALIVE_fnc_dump;
		_faction;
	};
};