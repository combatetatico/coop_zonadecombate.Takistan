...
Examples:
(begin example)
[["Outpost_Restrepo", getPos player, "MIL"], ["WEST"]] call SF_fnc_createObjective;
[["Bosovo", getMarkerPos "Town15"], ["WEST"]] call SF_fnc_createObjective;
[["DefendTown_3", _defendPos, "CIV", 250, 35, "defend"], ["BLU_F"]] call SF_fnc_createObjective;
[["AssaultPos_4", _assaultPos, "MIL", 300, 50, "attack"], ["BLU_F"]] call SF_fnc_createObjective;
(end)
...

{
...

	{
		...
		if (_identifier in ([_side] + _factions)) then {
			...
			["SF_fnc_createObjective: Registering %1 for %2", (_objectiveParams select 1) select 0, _identifier] call ALIVE_fnc_dump;
		};
